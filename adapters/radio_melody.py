...
def log_radio_event(radio_name, text):
    now = datetime.now(ZoneInfo("Europe/Bratislava"))
    timestamp = now.strftime("%d.%m.%Y %H:%M:%S")
    print(f"[{timestamp}] [{radio_name}] {text}")
...
def get_current_song():
    try:
        ...
        log_radio_event(radio_name, f"Zachytená skladba: {song_title}")
        return {
            "data": data,
        ...
        log_radio_event("Rádio Melody", f"Chyba pri získavaní skladby: {e}")
        return {
            "data": {},
            "recorded_at": datetime.now(ZoneInfo("Europe/Bratislava")).strftime("%d.%m.%Y %H:%M:%S"),
            "raw_valid": False,
    ...
    try:
        async with websockets.connect(LISTENERS_WS) as ws:
            while (datetime.now(ZoneInfo("Europe/Bratislava")) - start).total_seconds() < duration:
                ...
                log_radio_event("Rádio Melody", f"Zachytení poslucháči: {listeners_count}")
                results.append({
                    "data": data,
                    "recorded_at": datetime.now(ZoneInfo("Europe/Bratislava")).strftime("%d.%m.%Y %H:%M:%S"),
                    "raw_valid": valid,
        ...
        log_radio_event("Rádio Melody", f"Chyba pri získavaní listeners: {e}")
    return results
